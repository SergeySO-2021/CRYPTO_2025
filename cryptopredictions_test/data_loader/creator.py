...
def preprocess(dataset, cfg, logger=None):
    if logger:
        logger.info("Preprocess: initial dataset shape %s", dataset.shape)
        logger.info("Preprocess: initial dataset head %s", dataset.head(3).to_dict(orient='records'))
    dataset['Date'] = pd.to_datetime(dataset['Date'])
    train_start = pd.to_datetime(cfg.dataset_loader.train_start_date)
    valid_end = pd.to_datetime(cfg.dataset_loader.valid_end_date)
    if logger:
        logger.info("Preprocess: train_start %s valid_end %s", train_start, valid_end)
    dataset = dataset[(dataset['Date'] > train_start) & (dataset['Date'] < valid_end)]
    if logger:
        logger.info("Preprocess: dataset length after date filter %d", len(dataset))
    ...
    if logger:
        logger.info("Preprocess: using features %s", features)

    dates = dataset['Date']
    df = dataset[features]
    if logger:
        logger.info("Preprocess: subset dataframe shape %s", df.shape)
    ...
    if logger:
        logger.info("Preprocess: rows before mean calc %d", len(df))
    try:
    ...
    if logger:
        logger.info("Preprocess: rows before dropna %d", len(df))
    df = df.dropna()
    if logger:
        logger.info("Preprocess: rows after dropna %d", len(df))
    if len(df) == 0:
        raise ValueError("Dataset is empty after dropping NaNs. Check input data and features configuration.")
