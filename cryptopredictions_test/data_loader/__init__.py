...
# Ленивая загрузка BitmexDataset
def _get_bitmex_dataset():
    try:
        from .Bitmex import BitmexDataset
        return BitmexDataset
    except ImportError:
        return None
...
def get_dataset(dataset_name, start_date, end_date, args):
    ...
    if dataset_name == 'CoinMarket':
        ...
        BitmexDataset = _get_bitmex_dataset()
        if BitmexDataset is None:
            raise ImportError("bitmex module not installed. Install it with: pip install bitmex")
